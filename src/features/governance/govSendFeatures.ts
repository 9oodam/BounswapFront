...
export const propose = async (
  ...
  try {
    await (governanceContract.methods.propose as any)(userAddress, [
...
export const vote = async (
  ...
  try {
    const { hasVoted } = await (governanceContract.methods.getReceipt as any)(
      id,
      userAddress
    ).call();
    if (hasVoted) {
    ...
    await (governanceContract.methods.vote as any)(
      id,
      userAddress,
      support
    ).send({
