...
const index: React.FC<InputTokenProps> = ({ tokenName, value, setInputAmount }) => {
  const borderLeft = tokenName ? "border-l-2" : "border-l-0";
  return (
    <div className={Divstyle.box}>
      <input onChange={(e) => {setInputAmount?.(e.target.value)}} className={Inputstyle.size} value={value}></input>
